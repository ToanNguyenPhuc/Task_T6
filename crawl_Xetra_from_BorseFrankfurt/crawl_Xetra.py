...
Xetra_listing = 'ChungKhoanDuc.csv'
data = pd.read_csv(Xetra_listing)
...
with open('Xetra_listing.csv','a',newline='') as file:
    writer = csv.writer(file)
    writer.writerow(columns_name)
def handleOnePage(name,ISIN):
    first_table = driver.find_element(By.XPATH,'/html/body/app-root/app-wrapper/div/div[2]/app-equity/div[2]/div[6]/div/app-widget-ipo-company-data/div/div/table/tbody')
    second_table = driver.find_element(By.XPATH,'/html/body/app-root/app-wrapper/div/div[2]/app-equity/div[2]/div[4]/div[1]/app-widget-corporate-information/div/div/table/tbody')
    ...
    with open('Xetra_listing.csv','a',newline='',encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(dict_save_to_file.values())    
for ii,ISIN in enumerate(ISINs):
    print(ISIN)
    path =  f'https://www.boerse-frankfurt.de/aktie/{str(ISIN).strip()}/unternehmensangaben'
    print(path)
    driver.get(path)
    time.sleep(5)
    try: 
